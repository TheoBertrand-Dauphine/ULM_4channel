...
for p in range(20):
    ...
    for k in range(patches_per_image):
        ...
        if p<5:
            ...
            image_to_save.save('./data_IOSTAR/test_images/images_IOSTAR/test_IOSTAR_{}.png'.format(patches_per_image*p + k+1))
        ...
        if p<5:
            ...
            plt.savefig('./data_IOSTAR/test_images/visualization_test/test_im_scatter{}.png'.format(patches_per_image*p + k+1))
        plt.clf()
        
        if p<5:
            savemat('./data_IOSTAR/val_images/IOSTAR_points/IOSTAR_points_{}.mat'.format(patches_per_image*p + k+1), f1)
        else:
            savemat('./data_IOSTAR/test_images/IOSTAR_points/IOSTAR_points_{}.mat'.format(patches_per_image*p + k+1), f1)
    

#%% Making IOSTAR validation and test sets
...
for p, index in enumerate(images_numbering[:11]):
    ...
    for k in range(patches_per_image):
        ...
        if p<5:
            ...
            plt.savefig('./data_IOSTAR/test_images/visualization_test/test_im_scatter{}.png'.format(patches_per_image*p + k + 1 + patches_per_image*20))
        plt.clf()
        ...
        if p<5:
            ...
            savemat('./data_IOSTAR/test_images/IOSTAR_points/IOSTAR_points_{}.mat'.format(patches_per_image*p + k + 1 + patches_per_image*20), f1)
