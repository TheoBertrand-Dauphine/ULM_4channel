...
im_frangi = frangi(im_tensor.min(axis=0).values.numpy(),sigmas = np.exp(np.linspace(np.log(.05),np.log(5),10)), beta=1, alpha=.5, gamma = .5)**0.5
im_frangi = im_frangi/im_frangi.max() 

batch['image'] = np.concatenate([torch.tensor(im_frangi).unsqueeze(0).numpy(), GT.unsqueeze(0),original_image])

batch_transform = transforms.Compose([ToTensor(), Padding(0), ToArray(), HeatMap(s=13, alpha=3.55, out_channels = 4), CenterCrop(target_size), ToTensor(), Padding(16)])

batch_rescaled = batch_transform(batch)

points = Detection_Model(model, batch_rescaled, threshold=0.3)

points_tensor = torch.tensor(points).long()[points[:,2]!=3,:]

im_tensor, points_tensor = batch_rescaled['image'][0], points_tensor[ (points_tensor**2).sum(dim=-1)>0,:].long()
# print(points_tensor)
...
[A, points_tensor_mod, theta_indices_mod] = Modify_Metric_and_Points(torch.tensor(lifted_im_array).permute([1,0,2]), points_tensor, theta_indices, decalage =25)
...
[D, L, hfmIn] = Compute_Distance_Matrix(1/(1+1000*W**2), points_tensor_mod, theta_indices_mod, alpha=0.1, xi=.5)     

#%% Visualization

curves, list_of_stacks, Tcsr_list, prim_dict, labels = Cluster_from_Distance(D, L, distance_threshold = .02)
...
# mask_cropped = transforms.CenterCrop(target_size)(torch.tensor(mask_th)).numpy()
mask_from_tree = mask_from_list_of_stacks(list_of_stacks, mask_th, dist=.01)



Seg_score = (mask_from_tree*mask_th).sum()/np.maximum(mask_from_tree,mask_th).sum()

# Seg_score = (mask_from_tree*mask_cropped).sum()/np.maximum(mask_from_tree,mask_cropped).sum()
print(Seg_score)
plt.imshow(np.stack([mask_th*mask_from_tree, 0.+((mask_th-mask_from_tree)>0),0.+((-mask_th+mask_from_tree)>0)]).transpose(1,2,0))
# plt.scatter((mask_th.shape[0]/original_image.shape[-1])*points_tensor_mod[:,1], (mask_th.shape[-2]/original_image.shape[1])*points_tensor_mod[:,0], c=points_tensor_mod[:,2], alpha=.5)
# %%
