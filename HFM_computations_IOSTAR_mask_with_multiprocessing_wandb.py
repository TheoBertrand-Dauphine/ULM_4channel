...
def Show_Curves(I, im_tensor, points_tensor, list_of_stacks=[], data='synthetic', show_metric=False, th_output = True):
    ...
    # plt.show()
    wandb.log({'curves' : wandb.Image(plt)})
...
def main(args):
    ...
    input_image = transforms.CenterCrop(512)(torch.tensor(np.array(Image.open('./data_IOSTAR/test_with_GT/images/sSTAR 02_ODC_Merged.jpg'))).permute([2,0,1]).unsqueeze(0)/255)
    GT = transforms.CenterCrop(512)(torch.tensor(np.array(Image.open('./data_IOSTAR/test_with_GT/GT/sSTAR 02_ODC_Merged.tif'))))
    ...
    im_frangi = frangi(im_tensor.min(axis=0).values.numpy(),sigmas = np.exp(np.linspace(np.log(.05),np.log(5),10)), beta=args.beta, alpha=.5, gamma = .5)**0.5
    ...
    landmarks_frame = loadmat('./data_IOSTAR/test_with_GT/landmarks/IOSTAR_points_02.mat')
    ...
    batch_transform = transforms.Compose([Padding(0), ToArray(), HeatMap(s=13, alpha=3.55, out_channels = 4), CenterCrop(target_size), ToTensor(), Rescale(256)])
    landmarks_frame = loadmat('./data_IOSTAR/test_with_GT/landmarks/IOSTAR_points_02.mat')
    ...
    im_tensor, points_tensor = batch_rescaled['image'][1], batch_rescaled['landmarks'][ (points_tensor**2).sum(dim=-1)>0,:].long()
    # print(points_tensor)
    ...
    [D, L, hfmIn] = Compute_Distance_Matrix(1/(1+1000*W**2), points_tensor_mod, theta_indices_mod, alpha=args.alpha, xi=args.xi)     
    ...
    plt.close('all')
    Show_Curves(W, batch_rescaled['image'][2:], points_tensor_mod, list_of_stacks, show_metric=False)
    plt.close('all')
    # plt.savefig('./figures/' + data + '/curves_output' + str(0) + str(args.threshold_metric) + datetime.now().strftime("%H:%M:%S") + '.png')
    plt.close()
    ...
    # points_array_3d = np.hstack([points_tensor_mod[:,:-1], theta_indices_mod.unsqueeze(1)])
    # viewer = napari.view_image((np.concatenate([W[:,:,Nt//2:], W[:,:,:Nt//2]], axis=2)))
    ...
    # mask_th = transforms.CenterCrop(target_size)(GT).numpy()
    mask_th = batch_rescaled['image'][1].numpy()
    mask_th = mask_th/mask_th.max()


    widths = np.logspace(-3,-2,40)
    F1_array = test_mask_with_multiple_widths(list_of_stacks, mask_th, mask_th, widths)

    print("best seg score : {}".format(F1_array.max()), 'best width is : {}'.format(widths[F1_array.argmax()]))

    wandb.log({'score': F1_array.max()})

    plt.figure(5)
    plt.semilogx(widths, F1_array)
    wandb.log({'F1 score list for different widths': wandb.Image(plt)})
    # plt.savefig('./figures/' + data + '/F1_score' + str(0) + str(args.threshold_metric) + datetime.now().strftime("%H:%M:%S") + '.png')
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decalage', type=int, default=0, help='point dsiaplacement in the neighborhood')
    parser.add_argument('--beta', type=float, default=.1, help='frangi input')
    parser.add_argument('--sigma', type=float, default=.1, help='frangi input')
    parser.add_argument('--eps', type=float, default=.1, help='threshold for metric cost definition')
