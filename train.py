...
def main(args,seed):
    ...
    wandb.login()
    wandb.init(name = model.state_dict(), "data" + args.data +"_epochs_{}".format(args.epochs) + "_size_" + str(train_dataset[0]['image'].shape[-1]) "_batch_{}".format(args.batch_size) + "_out_channels_{}".format(args.out_channels) + '_alpha_' + str(args.alpha))
